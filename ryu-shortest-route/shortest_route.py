...
#        print ip_pkt
        #use ARP packet to get access_table	
        if arp_pkt:
                    ...
                    for port in self.access_ports[dpid]:
                        if (dpid, port) not in self.access_table.keys():
                            ...
                            out = parser.OFPPacketOut(
                                ...
                                actions=actions, data=msg.data)
                            datapath.send_msg(out)
	#use ip packet to get access_table
        if ip_pkt:
            ...
            # record the access info
            if in_port in self.access_ports[datapath.id]:
                self.access_table[(datapath.id, in_port)] = ip_src_ip

            result = self.get_host_location(ip_dst_ip)
            if result:  # host record in access table.
                datapath_dst, out_port = result[0], result[1]
                actions = [parser.OFPActionOutput(out_port)]
                datapath = self.datapaths[datapath_dst]
